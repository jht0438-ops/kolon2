...
# 건설 내부 후보 검증표: 공개자료에서 직접 확인 가능한 증거만 사용
construction_evidence = pd.DataFrame({
    "후보": ["산업·하이테크", "환경·수처리", "풍력·신재생"],
    "실제 사업/수주 근거": [
        "대한항공 엔진정비공장 등 산업시설 수행 + 하이테크 조직 확대",
        "환경공사 수행 + 반도체 폐수 특정물질 처리기술 R&D",
        "2026년 1Q 울진 기성 풍력단지 770억원 신규수주"
    ],
    "전담역량 근거": [
        "하이테크사업 조직 신설·전문인력 채용 확대",
        "ECO·수처리 수행경험 + 수처리 R&D",
        "신재생에너지·풍력 개발/EPC/운영 경험"
    ],
    "재무제표상 개별 수익성": ["미공시", "미공시", "미공시"],
    "판단": ["후보 유지", "후보 유지", "후보 유지"]
})

# 최종 제언은 앞 탭에서 확인한 증거를 다시 연결하도록 구성
allocation_logic = pd.DataFrame({
    "영역": ["산업·하이테크", "환경·수처리", "레저·AM", "풍력·신재생"],
    "앞선 분석에서 확인한 근거": [
        "비주택 확대 + 실제 산업시설 프로젝트 + 하이테크 조직·채용 확대",
        "환경공사 수행 + 반도체 폐수처리 R&D로 산업시설과 기술 연계 가능",
        "2026년 1Q 매출 649억원·전년동기 대비 +344.5%·영업이익 흑자전환",
        "개발/EPC/운영 경험 + 2026년 1Q 울진 기성 풍력 770억원 신규수주"
    ],
    "재무적 의미": [
        "주택 외 수주원 확대",
        "기존 환경기술을 산업용 수요와 연결",
        "건설 프로젝트와 다른 운영수익원 보완",
        "실제 신규수주가 확인된 친환경에너지 성장옵션"
    ],
    "제언 가능한 자원": [
        "전문인력·수주/공사관리 역량",
        "R&D·기술인력·산업시설 영업 연계",
...
with tab6:
    st.subheader("⑥ 건설 내부 비주택 후보: 무엇이 실제 성장축으로 확인되는가?")
    st.write("하이테크·수처리·풍력은 건설부문 안에 포함돼 개별 영업이익률이 공시되지 않습니다. 따라서 수익성을 임의로 순위화하지 않고, 실제 수주·프로젝트·조직·R&D라는 네 가지 증거를 확인합니다.")

    st.markdown("### 1. 세 후보의 검증 결과")
    st.dataframe(construction_evidence, hide_index=True, use_container_width=True)

    st.markdown("### 2. 산업·하이테크: '관심 분야'가 아니라 실제 역량 확대가 확인되는가?")
    h1,h2,h3 = st.columns(3)
    h1.metric("2024 전체 신규수주", "약 4.2조원")
    h2.metric("2024 비주택 신규수주", "약 2.3조원", "전체의 절반 이상")
    h3.metric("하이테크 조직", "신설·채용 확대")
    st.markdown("""
    <div class="kolon-card">
    2024년 비주택 신규수주가 2.3조원을 넘어 전체 신규수주의 절반 이상을 차지했고, 회사는 수주경쟁력 강화를 위해 <b>하이테크 사업조직을 신설</b>하고 관련 공사·기술관리 인력 채용을 확대했습니다.
    기존 건설계약에서도 대한항공 엔진정비공장 등 산업시설 프로젝트가 확인됩니다.<br><br>
    <b>판단:</b> 단순 시장전망이 아니라 <b>수주 확대 → 실제 프로젝트 → 조직·인력 확대</b>가 함께 확인되므로 성장자원 배분 후보로 유지합니다.
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### 3. 환경·수처리: 기존 사업과 기술투자가 연결되는가?")
    st.markdown("""
    <div class="kolon-card">
    코오롱글로벌은 기존 환경·수처리 공사를 수행해 왔고, 2026년 R&D에서는 <b>반도체 폐수의 특정물질 처리기술</b> 등 산업용 수처리 기술개발이 확인됩니다.
    이는 환경사업을 단순 공공 환경공사에 두는 것이 아니라 산업시설 수요와 연결할 수 있는 기술적 기반을 강화하는 흐름으로 볼 수 있습니다.<br><br>
    <b>판단:</b> <b>기존 수행경험 + 현재 R&D + 산업시설과의 연결 가능성</b>이 동시에 확인되므로 후보로 유지합니다.
    단, 별도 매출·영업이익이 공시되지 않으므로 하이테크보다 수익성이 높다고 판단하지 않습니다.
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### 4. 풍력: 장기 가능성만 있는가, 실제 수주도 있는가?")
    w1,w2,w3 = st.columns(3)
    w1.metric("2026년 1Q 전체 신규수주", "4,044억원", "전년동기 대비 +19%")
    w2.metric("울진 기성 풍력단지", "770억원", "2026년 1Q 신규수주")
    w3.metric("사업경험", "개발·EPC·운영")
    st.markdown("""
    <div class="kolon-card">
    풍력은 과거 사업경험만 있는 후보가 아닙니다. 2026년 1분기 전체 신규수주 4,044억원 가운데 <b>울진 기성 풍력단지 770억원</b>의 신규수주가 확인됩니다.
    여기에 기존 개발·EPC·운영 경험이 있다는 점을 결합하면 실제 사업화가 이어지고 있는 후보로 볼 수 있습니다.<br><br>
    <b>판단:</b> 실제 신규수주까지 확인되므로 후보에서 제외할 근거는 없습니다. 다만 개별 사업 수익성이 공시되지 않아 <b>다른 비주택 후보보다 우월하다고 순위를 매기지는 않습니다.</b>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("""
    <div class="kolon-note"><b>6번 탭 결론</b><br><br>
    건설 내부에서는 세 후보 모두 객관적 근거가 확인돼 단순 탈락시키기 어렵습니다. 대신 역할이 다릅니다.<br>
    • 산업·하이테크: 비주택 수주 확대와 조직·인력 확충이 동시에 확인되는 <b>수주 확대 축</b><br>
    • 환경·수처리: 기존 환경사업과 현재 R&D를 산업시설에 연결할 수 있는 <b>기술 차별화 축</b><br>
    • 풍력: 기존 밸류체인 경험에 2026년 실제 신규수주까지 확인되는 <b>친환경에너지 축</b><br><br>
    따라서 최종 탭에서는 임의의 1등을 정하기보다 <b>각 근거에 맞는 자원을 무엇에 배분할지</b> 제언합니다.
    </div>
    """, unsafe_allow_html=True)

with tab7:
    st.subheader("⑦ 검증 결과 → 자원배분 제언")
    st.write("이 탭에서는 새로운 근거를 추가하지 않습니다. 4~6번 탭에서 확인한 사실이 어떤 자원배분으로 이어지는지만 연결합니다.")

    st.markdown("### 1. 결론이 어디서 나왔는지 먼저 확인")
    st.dataframe(allocation_logic, hide_index=True, use_container_width=True)

    st.markdown("### 2. 근거에 맞춰 자원의 종류를 다르게 배분")
    c1,c2 = st.columns(2)
    with c1:
        st.markdown("""
        <div class="kolon-card"><b>① 산업·하이테크 — 수주·수행 역량 강화</b><br><br>
        <b>근거</b><br>
        비주택 신규수주 확대 → 실제 산업시설 프로젝트 → 하이테크 조직 신설 및 관련 인력 확대<br><br>
        <b>그래서 무엇에 투자?</b><br>
        하이테크 전문인력, 원가·공정관리 역량, 기업고객 수주 네트워크에 자원을 배분합니다.<br><br>
        <b>왜?</b><br>
        이미 나타난 비주택 수주 흐름을 실제 매출과 이익으로 전환하려면 수주 자체뿐 아니라 수행역량이 함께 커져야 하기 때문입니다.
        </div>
        """, unsafe_allow_html=True)
    with c2:
        st.markdown("""
        <div class="kolon-card"><b>② 환경·수처리 — 기술 차별화 투자</b><br><br>
        <b>근거</b><br>
        기존 환경·수처리 수행경험 → 반도체 폐수 특정물질 처리 R&D → 산업시설 사업과 연결 가능<br><br>
        <b>그래서 무엇에 투자?</b><br>
        수처리 R&D와 기술인력, 하이테크 수주조직과 환경기술조직의 연계에 자원을 배분합니다.<br><br>
        <b>왜?</b><br>
        단순 시공 경쟁이 아니라 코오롱글로벌이 이미 가진 환경기술을 산업시설 수주 경쟁력으로 연결할 수 있기 때문입니다.
        </div>
        """, unsafe_allow_html=True)
    ...
    with c3:
        st.markdown("""
        <div class="kolon-card"><b>③ 레저·AM — 운영수익 기반 확대</b><br><br>
        <b>근거</b><br>
        2026년 1Q 합산 매출 649억원 → 전년동기 대비 +344.5% → 영업이익 흑자전환 → 회사도 운영수익 축으로 제시<br><br>
        <b>그래서 무엇에 투자?</b><br>
        운영효율화와 장기 위탁운영 계약 확보, 건설 준공 이후 AM 서비스로 이어지는 고객 연계에 자원을 배분합니다.<br><br>
        <b>왜?</b><br>
        건설과 다른 방식으로 실제 실적 기여가 시작됐기 때문에 건설경기 변동을 보완하는 수익원으로 검증할 가치가 있기 때문입니다.
        </div>
        """, unsafe_allow_html=True)
    with c4:
        st.markdown("""
        <div class="kolon-card"><b>④ 풍력 — 실제 수주 기반의 선택적 확대</b><br><br>
        <b>근거</b><br>
        개발·EPC·운영 경험 → 2026년 1Q 울진 기성 풍력 770억원 신규수주<br><br>
        <b>그래서 무엇에 투자?</b><br>
        후속 개발 파이프라인과 사업성 검증, EPC 수행역량에 선택적으로 자원을 배분합니다.<br><br>
        <b>왜?</b><br>
        실제 신규수주는 확인됐지만 사업별 수익성이 별도 공시되지 않으므로 무조건적인 대규모 확대보다 프로젝트별 사업성을 확인하며 확장하는 것이 공개자료에 맞는 결론입니다.
        </div>
        """, unsafe_allow_html=True)

    st.markdown("""
    <div class="kolon-note"><b>최종 제언</b><br><br>
    분석 결과는 하나의 사업에 모든 자원을 집중하라는 결론이 아닙니다.<br><br>
    <b>산업·하이테크</b>에는 비주택 수주 확대를 실제 매출로 연결할 <b>전문인력·수행역량</b>,
    <b>환경·수처리</b>에는 산업시설과 결합할 <b>기술·R&D</b>,
    <b>레저·AM</b>에는 건설 변동성을 보완할 <b>운영효율화·장기계약</b>,
    <b>풍력</b>에는 실제 수주를 후속 사업으로 연결할 <b>개발 파이프라인과 사업성 검증</b>에 자원을 배분하는 방향을 제언합니다.<br><br>
    핵심은 <b>주택을 단순히 줄이는 것이 아니라, 기존 건설역량을 활용하는 비주택 성장축과 건설 밖 운영수익 축을 함께 강화하는 것</b>입니다.
    </div>
    """, unsafe_allow_html=True)

    st.caption("주의: 공개자료에서는 하이테크·수처리·풍력의 개별 영업이익률이 분리 공시되지 않습니다. 따라서 본 분석은 사업별 수익성 순위를 제시하지 않고, 확인 가능한 수주·실적·R&D·조직 근거에 맞춰 자원배분 방향을 제언합니다.")
